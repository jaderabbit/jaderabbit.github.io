...
(function () {
  var GOL = {
    ...
    zoom: {
      current: 0,
      schedule: false,
      schemes: [// { columns : 100, rows : 48, cellSize : 8 },
      {
        columns: 280,
        rows: 140,
        cellSize: 6
    ...
    colors: {
      ...
      schemes: [{
        dead: '#181824',
        trail: ['#6feae6', '#90e0d6', '#90dce0'],
        alive: ['#666666', '#999999', '#ccccccc', '#eeeeee']
      }, {
        dead: '#FFFFFF',
    ...
    init: function init() {
      try {
        ...
        this.randomState(); // Load state from URL
    ...
    loadConfig: function loadConfig() {
      ...
      this.autoplay = true;
      this.trail.current = this.helpers.getUrlParameter('trail') === '1' ? false : this.trail.current; // Initial color config
      ...
      this.grid.current = this.grid.schemes.length - 1;
     ...
     */
    keepDOMElements: function keepDOMElements() {
    ...
    registerEvents: function registerEvents() {
      // Keyboard Events
      this.helpers.registerEvent(document.body, 'keyup', this.handlers.keyboard, false); // Controls
      ...
      this.helpers.registerEvent(document.getElementById('buttonExport'), 'click', this.handlers.buttons.export_, false); // Layout
    ...
    nextStep: function nextStep() {
      var i, x, y, r, liveCellNumber, algorithmTime, guiTime; // Algorithm run

      algorithmTime = new Date();
      liveCellNumber = GOL.listLife.nextGeneration();
      algorithmTime = new Date() - algorithmTime; // Canvas run
      ...
      guiTime = new Date() - guiTime; // Pos-run updates
      // Clear Trail
      ...
      GOL.times.gui = GOL.times.gui * (1 - r) + guiTime * r; // Flow Control
      ...
      if (GOL.running) {
        ...
        if (GOL.waitTime > 0) setTimeout(function () {
          animateFrame();
        }, GOL.waitTime);else animateFrame();
      } else {
    ...
    handlers: {
       ...
       *
       */
      canvasMouseDown: function canvasMouseDown(event) {
       ...
       *
       */
      canvasMouseUp: function canvasMouseUp() {
       ...
       *
       */
      canvasMouseMove: function canvasMouseMove(event) {
      ...
      buttons: {
        ...
        run: function run() {
          ...
          if (GOL.running) {
            GOL.nextStep(); //document.getElementById('buttonRun').value = 'Stop';
          } else {//document.getElementById('buttonRun').value = 'Run';
            }
        },
        ...
        clear: function clear() {
          if (GOL.running) {
            ...
            GOL.running = false; //document.getElementById('buttonRun').value = 'Run';
          } else {
            GOL.cleanUp();
          }
        },
        ...
        colors: function colors() {
          GOL.colors.current = (GOL.colors.current + 1) % GOL.colors.schemes.length; //GOL.element.messages.layout.innerHTML = 'Color Scheme #' + (GOL.colors.current + 1);
         ...
         */
        grid: function grid() {
          GOL.grid.current = (GOL.grid.current + 1) % GOL.grid.schemes.length; //GOL.element.messages.layout.innerHTML = 'Grid Scheme #' + (GOL.grid.current + 1);
        ...
        export_: function export_() {
          ...
          for (i = 0; i < GOL.listLife.actualState.length; i++) {
            cellState += '{"' + GOL.listLife.actualState[i][0] + '":['; //cellState += '{"one":[';
          ...
          if (cellState.length !== 0) {
            ...
            params = '?autoplay=0' + '&trail=' + (GOL.trail.current ? '1' : '0') + '&grid=' + (GOL.grid.current + 1) + '&colors=' + (GOL.colors.current + 1) + '&zoom=' + (GOL.zoom.current + 1) + '&s=[' + cellState + ']';
    ...
    canvas: {
      ...
      clearWorld: function clearWorld() {
        var i, j; // Init ages (Canvas reference)
      ...
      drawWorld: function drawWorld() {
        ...
        this.canvas.setAttribute('height', this.height); // Fill background
       ...
       * setNoGridOn
       */
      setNoGridOn: function setNoGridOn() {
       ...
       * setNoGridOff
       */
      setNoGridOff: function setNoGridOff() {
        this.cellSize = GOL.zoom.schemes[GOL.zoom.current].cellSize;
        this.cellSpace = 1;
      },
       ...
       * drawCell
       */
      drawCell: function drawCell(i, j, alive) {
        if (alive) {
          if (this.age[i][j] > -1) this.context.fillStyle = GOL.colors.schemes[GOL.colors.current].alive[this.age[i][j] % GOL.colors.schemes[GOL.colors.current].alive.length];
        } else {
       ...
       * keepCellAlive
       */
      keepCellAlive: function keepCellAlive(i, j) {
       ...
       * changeCelltoAlive
       */
      changeCelltoAlive: function changeCelltoAlive(i, j) {
        if (i >= 0 && i < GOL.columns && j >= 0 && j < GOL.rows) {
          this.age[i][j] = 1;
          this.drawCell(i, j, true);
        }
       ...
       * changeCelltoDead
       */
      changeCelltoDead: function changeCelltoDead(i, j) {
        if (i >= 0 && i < GOL.columns && j >= 0 && j < GOL.rows) {
    ...
    listLife: {
      ...
      */
      nextGeneration: function nextGeneration() {
        ...
        for (i = 0; i < this.actualState.length; i++) {
          ...
          for (j = 1; j < this.actualState[i].length; j++) {
            ...
            y = this.actualState[i][0]; // Possible dead neighbours

            deadNeighbours = [[x - 1, y - 1, 1], [x, y - 1, 1], [x + 1, y - 1, 1], [x - 1, y, 1], [x + 1, y, 1], [x - 1, y + 1, 1], [x, y + 1, 1], [x + 1, y + 1, 1]]; // Get number of live neighbours and remove alive neighbours from deadNeighbours

            neighbours = this.getNeighboursFromAlive(x, y, i, deadNeighbours); // Join dead neighbours to check list
      ...
      /**
             *
             */
      getNeighboursFromAlive: function getNeighboursFromAlive(x, y, i, possibleNeighboursList) {
        var neighbours = 0,
            k; // Top
       ...
       *
       */
      removeCell: function removeCell(x, y, state) {
        var i, j;
       ...
       *
       */
      addCell: function addCell(x, y, state) {
        if (state.length === 0) {
          state.push([y, x]);
          return;
        }
    ...
    helpers: {
       ...
       * Register Event
       */
      registerEvent: function registerEvent(element, event, handler, capture) {
